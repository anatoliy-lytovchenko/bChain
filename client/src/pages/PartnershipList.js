...
class PartnershipList extends Component {
  ...
  confirmDialogDelete(id) {
    this.props.actionsPartnership.deletePartnership(this.state.idDelete).then(data => {
      this.props.actionsPartnership.loadPartnershipList();
      this.setState({ openDialogDelete: false, idDelete: null });
    });
  }
            ...
            {this.props.list.map(row => (
              <TableRow key={row._id}>
                <TableCell component="th" scope="row">
                  <Link to={"/partnerships/" + row._id} key={row._id}>
                    {row._id}
                  </Link>
                </TableCell>
...
function mapStateToProps(state, ownProps) {
  return {
    list: state.PartnershipListReducer.listPartnership
  };
}
